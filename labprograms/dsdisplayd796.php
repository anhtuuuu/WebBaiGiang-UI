...
<!-- Mirrored from studyglance.in/labprograms/dsdisplay.php?url1=ds/CLL.c&url2=&url3=&url4=&title=Write%20a%20program%20that%20uses%20functions%20to%20perform%20the%20following%20operations%20on%20Circular%20Linked%20List.:i)%20Creation%20%20%20%20%20ii)%20Insertion%20%20%20%20iii)%20Deletion%20%20%20%20iv)%20Traversal%20%20%20&opurl1=ds/outputs/clla.png&opurl2=ds/outputs/cllb.png&opurl3=&opurl4= by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 28 May 2025 03:51:12 GMT -->
...
<div class="panel panel-default">
 ...
 <!--<iframe src="https://docs.google.com/gview?url=http://localhost/studyglance/XML.pdf&embedded=true" style="width:600px; height:500px;" frameborder="0"></iframe>-->
  ...
  <h2>Aim: </h2>
  <p>Write a program that uses functions to perform the following operations on Circular Linked List.:i) Creation     ii) Insertion    iii) Deletion    iv) Traversal   </p>
  <h2>Source Code: </h2>
  <h3>CLL.c</h3>
<pre><code class="language-c"><code><span style="color: #000000">
#include&lt;stdio.h&gt;<br />#include&lt;stdlib.h&gt;<br />struct&nbsp;node<br />{<br />int&nbsp;data;<br />struct&nbsp;node&nbsp;*next;<br />};<br />struct&nbsp;node&nbsp;*head;<br />void&nbsp;beginsert&nbsp;();<br />void&nbsp;lastinsert&nbsp;();<br />void&nbsp;randominsert();<br />void&nbsp;begin_delete();<br />void&nbsp;last_delete();<br />void&nbsp;random_delete();<br />void&nbsp;display();<br />void&nbsp;search();<br />void&nbsp;main&nbsp;()<br />{<br />int&nbsp;choice&nbsp;=0;<br />while(choice&nbsp;!=&nbsp;7)<br />{<br />printf("\n*********Main&nbsp;Menu*********\n");<br />printf("\nChoose&nbsp;one&nbsp;option&nbsp;from&nbsp;the&nbsp;following&nbsp;list&nbsp;...\n");<br />printf("\n===============================================\n");<br />printf("\n1.Insert&nbsp;in&nbsp;begining\n2.Insert&nbsp;at&nbsp;last\n3.Delete&nbsp;from&nbsp;Beginning\n4.Delete&nbsp;from&nbsp;last\n5.Search&nbsp;for&nbsp;an&nbsp;element\n6.Show\n7.Exit\n");<br />printf("\nEnter&nbsp;your&nbsp;choice?\n");<br />scanf("\n%d",&amp;choice);<br />switch(choice)<br />{<br />case&nbsp;1:<br />beginsert();<br />break;<br />case&nbsp;2:<br />lastinsert();<br />break;<br />case&nbsp;3:<br />begin_delete();<br />break;<br />case&nbsp;4:<br />last_delete();<br />break;<br />case&nbsp;5:<br />search();<br />break;<br />case&nbsp;6:<br />display();<br />break;<br />case&nbsp;7:<br />exit(0);<br />break;<br />default:<br />printf("Please&nbsp;enter&nbsp;valid&nbsp;choice..");<br />}<br />}<br />}<br />void&nbsp;beginsert()<br />{<br />struct&nbsp;node&nbsp;*ptr,*temp;<br />int&nbsp;item;<br />ptr&nbsp;=&nbsp;(struct&nbsp;node&nbsp;*)malloc(sizeof(struct&nbsp;node));<br />if(ptr&nbsp;==&nbsp;NULL)<br />{<br />printf("\nOVERFLOW");<br />}<br />else<br />{<br />printf("\nEnter&nbsp;the&nbsp;node&nbsp;data?");<br />scanf("%d",&amp;item);<br />ptr&nbsp;-&gt;&nbsp;data&nbsp;=&nbsp;item;<br />if(head&nbsp;==&nbsp;NULL)<br />{<br />head&nbsp;=&nbsp;ptr;<br />ptr&nbsp;-&gt;&nbsp;next&nbsp;=&nbsp;head;<br />}<br />else<br />{<br />temp&nbsp;=&nbsp;head;<br />while(temp-&gt;next&nbsp;!=&nbsp;head)<br />temp&nbsp;=&nbsp;temp-&gt;next;<br />ptr-&gt;next&nbsp;=&nbsp;head;<br />temp&nbsp;-&gt;&nbsp;next&nbsp;=&nbsp;ptr;<br />head&nbsp;=&nbsp;ptr;<br />}<br />printf("\nnode&nbsp;inserted\n");<br />}<br />}<br />void&nbsp;lastinsert()<br />{<br />struct&nbsp;node&nbsp;*ptr,*temp;<br />int&nbsp;item;<br />ptr&nbsp;=&nbsp;(struct&nbsp;node&nbsp;*)malloc(sizeof(struct&nbsp;node));<br />if(ptr&nbsp;==&nbsp;NULL)<br />{<br />printf("\nOVERFLOW\n");<br />}<br />else<br />{<br />printf("\nEnter&nbsp;Data?");<br />scanf("%d",&amp;item);<br />ptr-&gt;data&nbsp;=&nbsp;item;<br />if(head&nbsp;==&nbsp;NULL)<br />{<br />head&nbsp;=&nbsp;ptr;<br />ptr&nbsp;-&gt;&nbsp;next&nbsp;=&nbsp;head;<br />}<br />else<br />{<br />temp&nbsp;=&nbsp;head;<br />while(temp&nbsp;-&gt;&nbsp;next&nbsp;!=&nbsp;head)<br />{<br />temp&nbsp;=&nbsp;temp&nbsp;-&gt;&nbsp;next;<br />}<br />temp&nbsp;-&gt;&nbsp;next&nbsp;=&nbsp;ptr;<br />ptr&nbsp;-&gt;&nbsp;next&nbsp;=&nbsp;head;<br />}<br /><br />printf("\nnode&nbsp;inserted\n");<br />}<br />}<br /><br />void&nbsp;begin_delete()<br />{<br />struct&nbsp;node&nbsp;*ptr;<br />if(head&nbsp;==&nbsp;NULL)<br />{<br />printf("\nUNDERFLOW");<br />}<br />else&nbsp;if(head-&gt;next&nbsp;==&nbsp;head)<br />{<br />head&nbsp;=&nbsp;NULL;<br />free(head);<br />printf("\nnode&nbsp;deleted\n");<br />}<br />else<br />{&nbsp;&nbsp;&nbsp;ptr&nbsp;=&nbsp;head;<br />while(ptr&nbsp;-&gt;&nbsp;next&nbsp;!=&nbsp;head)<br />ptr&nbsp;=&nbsp;ptr&nbsp;-&gt;&nbsp;next;<br />ptr-&gt;next&nbsp;=&nbsp;head-&gt;next;<br />free(head);<br />head&nbsp;=&nbsp;ptr-&gt;next;<br />printf("\nnode&nbsp;deleted\n");<br />}<br />}<br />void&nbsp;last_delete()<br />{<br />struct&nbsp;node&nbsp;*ptr,&nbsp;*preptr;<br />if(head==NULL)<br />{<br />printf("\nUNDERFLOW");<br />}<br />else&nbsp;if&nbsp;(head&nbsp;-&gt;next&nbsp;==&nbsp;head)<br />{<br />head&nbsp;=&nbsp;NULL;<br />free(head);<br />printf("\nnode&nbsp;deleted\n");<br />}<br />else<br />{<br />ptr&nbsp;=&nbsp;head;<br />while(ptr&nbsp;-&gt;next&nbsp;!=&nbsp;head)<br />{<br />preptr=ptr;<br />ptr&nbsp;=&nbsp;ptr-&gt;next;<br />}<br />preptr-&gt;next&nbsp;=&nbsp;ptr&nbsp;-&gt;&nbsp;next;<br />free(ptr);<br />printf("\nnode&nbsp;deleted\n");<br />}<br />}<br />void&nbsp;search()<br />{<br />struct&nbsp;node&nbsp;*ptr;<br />int&nbsp;item,i=0,flag=1;<br />ptr&nbsp;=&nbsp;head;<br />if(ptr&nbsp;==&nbsp;NULL)<br />{<br />printf("\nEmpty&nbsp;List\n");<br />}<br />else<br />{<br />printf("\nEnter&nbsp;item&nbsp;which&nbsp;you&nbsp;want&nbsp;to&nbsp;search?\n");<br />scanf("%d",&amp;item);<br />if(head&nbsp;-&gt;data&nbsp;==&nbsp;item)<br />{<br />printf("item&nbsp;found&nbsp;at&nbsp;location&nbsp;%d",i+1);<br />flag=0;<br />}<br />else<br />{<br />while&nbsp;(ptr-&gt;next&nbsp;!=&nbsp;head)<br />{<br />if(ptr-&gt;data&nbsp;==&nbsp;item)<br />{<br />printf("item&nbsp;found&nbsp;at&nbsp;location&nbsp;%d&nbsp;",i+1);<br />flag=0;<br />break;<br />}<br />else<br />{<br />flag=1;<br />}<br />i++;<br />ptr&nbsp;=&nbsp;ptr&nbsp;-&gt;&nbsp;next;<br />}<br />}<br />if(flag&nbsp;!=&nbsp;0)<br />{<br />printf("Item&nbsp;not&nbsp;found\n");<br />}&nbsp;}&nbsp;}<br /><br />void&nbsp;display()<br />{<br />struct&nbsp;node&nbsp;*ptr;<br />ptr=head;<br />if(head&nbsp;==&nbsp;NULL)<br />{<br />printf("\nnothing&nbsp;to&nbsp;print");<br />}<br />else<br />{<br />printf("\n&nbsp;printing&nbsp;values&nbsp;...&nbsp;\n");<br />while(ptr&nbsp;-&gt;&nbsp;next&nbsp;!=&nbsp;head)<br />{<br />printf("%d\n",&nbsp;ptr&nbsp;-&gt;&nbsp;data);<br />ptr&nbsp;=&nbsp;ptr&nbsp;-&gt;&nbsp;next;<br />}<br />printf("%d\n",&nbsp;ptr&nbsp;-&gt;&nbsp;data);<br />}<br />}<br /></span>
</code></code></pre><br>
<!--<br />
...
-->
<h2>Output: </h2>
<img src="ds/outputs/clla.png" alt="image" class="img-rounded img-responsive"/><br/>
<img src="ds/outputs/cllb.png" alt="image" class="img-rounded img-responsive"/><br/>
<!--<br />
