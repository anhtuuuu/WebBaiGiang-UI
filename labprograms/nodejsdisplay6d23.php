...
<!-- Mirrored from studyglance.in/labprograms/nodejsdisplay.php?url1=nodejs/exp7.php&title=Develop%20an%20express%20web%20application%20that%20can%20interact%20with%20REST%20API%20to%20perform%20CRUD%20operations%20on%20student%20data.%20(Use%20Postman) by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 28 May 2025 03:48:51 GMT -->
...
<h2>Aim: </h2>
  <h1 style="font-size:18px;color:black;"><span style='font-size:25px;color:#ff3333;'>&#9755;</span> &nbsp;Develop an express web application that can interact with REST API to perform CRUD operations on student data. (Use Postman) </h1>
  <h2>Solution : </h2>	
...
<li> Open that folder in the vscode by entering <em>code</em>.</li>
<li> Next in the terminal we need to install all the packages we need, so we mainly use express and sqlite3. </li>
<li> The Command to install express and sqlite3 is
<pre> 
npm install express sqlite3 

</pre></li>
</ul>
<p>Then create file named as the <strong>app.js</strong> and <strong>db.js</strong> </p>

 <h3>db.js</h3>
  <pre><code class="language-js"><code><span style="color: #000000">
const&nbsp;sqlite3&nbsp;=&nbsp;require('sqlite3').verbose();<br /><br />//&nbsp;Function&nbsp;to&nbsp;initialize&nbsp;the&nbsp;database&nbsp;schema<br />function&nbsp;initializeDatabase()&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const&nbsp;db&nbsp;=&nbsp;new&nbsp;sqlite3.Database('./mydatabase.db',&nbsp;(err)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;else&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('Connected&nbsp;to&nbsp;the&nbsp;SQLite&nbsp;database.');<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;createStudentsTable(db);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br /><br />&nbsp;&nbsp;&nbsp;&nbsp;//&nbsp;Close&nbsp;the&nbsp;database&nbsp;connection&nbsp;when&nbsp;the&nbsp;Node&nbsp;process&nbsp;exits<br />&nbsp;&nbsp;&nbsp;&nbsp;process.on('exit',&nbsp;()&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;db.close((err)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;else&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('Disconnected&nbsp;from&nbsp;the&nbsp;SQLite&nbsp;database.');<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;});<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />}<br />//&nbsp;Function&nbsp;to&nbsp;create&nbsp;the&nbsp;'students'&nbsp;table&nbsp;if&nbsp;it&nbsp;doesn't&nbsp;exist<br />function&nbsp;createStudentsTable(db)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const&nbsp;createTableQuery&nbsp;=&nbsp;`<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;CREATE&nbsp;TABLE&nbsp;IF&nbsp;NOT&nbsp;EXISTS&nbsp;students&nbsp;(<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;id&nbsp;INTEGER&nbsp;PRIMARY&nbsp;KEY&nbsp;AUTOINCREMENT,<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;name&nbsp;TEXT,<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;age&nbsp;INTEGER,<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;grade&nbsp;TEXT<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;);<br />&nbsp;&nbsp;&nbsp;&nbsp;`;<br /><br />&nbsp;&nbsp;&nbsp;&nbsp;db.run(createTableQuery,&nbsp;(err)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;else&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('The&nbsp;students&nbsp;table&nbsp;has&nbsp;been&nbsp;created&nbsp;or&nbsp;already&nbsp;exists.');<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />}<br /><br />module.exports&nbsp;=&nbsp;{&nbsp;initializeDatabase&nbsp;};</span>
</code>  </code></pre><br>
<p>when we execute both the <strong>db.js</strong> then the database will be created that is <strong>mydatabase.db</strong> </p>
 <h3>app.js</h3>
  <pre><code class="language-js"><code><span style="color: #000000">
const&nbsp;express&nbsp;=&nbsp;require('express');<br />const&nbsp;sqlite3&nbsp;=&nbsp;require('sqlite3');<br />const{&nbsp;initializeDatabase&nbsp;}&nbsp;=&nbsp;require('./db');&nbsp;<br />const&nbsp;app&nbsp;=&nbsp;express();<br />const&nbsp;port&nbsp;=&nbsp;3000;<br /><br />//&nbsp;Connect&nbsp;to&nbsp;SQLite&nbsp;database<br />const&nbsp;db&nbsp;=&nbsp;new&nbsp;sqlite3.Database('./mydatabase.db',&nbsp;(err)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;}&nbsp;else&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;console.log('Connected&nbsp;to&nbsp;the&nbsp;SQLite&nbsp;database.');<br />&nbsp;&nbsp;&nbsp;&nbsp;}<br />});<br /><br />//&nbsp;Middleware&nbsp;to&nbsp;parse&nbsp;request&nbsp;body&nbsp;as&nbsp;JSON<br />app.use(express.json());<br /><br />app.get('/',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;res.send('Welcome&nbsp;to&nbsp;the&nbsp;Student');<br />});&nbsp;<br /><br />//&nbsp;Get&nbsp;all&nbsp;Students<br />app.get('/students',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;db.all('SELECT&nbsp;*&nbsp;FROM&nbsp;students',&nbsp;[],&nbsp;(err,&nbsp;rows)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.json(rows);<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />});<br /><br />//&nbsp;Get&nbsp;a&nbsp;single&nbsp;student&nbsp;by&nbsp;id<br />app.get('/students/:id',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const&nbsp;id&nbsp;=&nbsp;req.params.id;<br />&nbsp;&nbsp;&nbsp;&nbsp;db.all('SELECT&nbsp;*&nbsp;FROM&nbsp;students&nbsp;WHERE&nbsp;id&nbsp;=&nbsp;?',&nbsp;[id],&nbsp;(err,&nbsp;row)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.json(row);<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />});<br /><br />//&nbsp;Create&nbsp;a&nbsp;new&nbsp;student<br />app.post('/students',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const{&nbsp;name,&nbsp;age,&nbsp;grade&nbsp;}&nbsp;=&nbsp;req.body;<br />&nbsp;&nbsp;&nbsp;&nbsp;db.run('INSERT&nbsp;INTO&nbsp;students&nbsp;(name,&nbsp;age,&nbsp;grade)&nbsp;VALUES&nbsp;(?,&nbsp;?,&nbsp;?)',&nbsp;[name,&nbsp;age,&nbsp;grade],&nbsp;function&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.status(201).json({&nbsp;id:this.lastID&nbsp;});<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />});<br /><br />//&nbsp;Update&nbsp;a&nbsp;student<br />app.put('/students/:id',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const&nbsp;id&nbsp;=&nbsp;req.params.id;<br />&nbsp;&nbsp;&nbsp;&nbsp;const{&nbsp;name,&nbsp;age,&nbsp;grade&nbsp;}&nbsp;=&nbsp;req.body;<br />&nbsp;&nbsp;&nbsp;&nbsp;db.run('UPDATE&nbsp;students&nbsp;SET&nbsp;name&nbsp;=&nbsp;?,&nbsp;age&nbsp;=&nbsp;?,&nbsp;grade&nbsp;=&nbsp;?&nbsp;WHERE&nbsp;id&nbsp;=&nbsp;?',&nbsp;[name,&nbsp;age,&nbsp;grade,&nbsp;id],&nbsp;function&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.json({&nbsp;updatedID:id&nbsp;});<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />});<br /><br />//&nbsp;Delete&nbsp;a&nbsp;student<br />app.delete('/students/:id',&nbsp;(req,&nbsp;res)&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;const&nbsp;id&nbsp;=&nbsp;req.params.id;<br />&nbsp;&nbsp;&nbsp;&nbsp;db.run('DELETE&nbsp;FROM&nbsp;students&nbsp;WHERE&nbsp;id&nbsp;=&nbsp;?',&nbsp;id,&nbsp;function&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;if&nbsp;(err)&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;console.error(err.message);<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;}<br />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;res.json({&nbsp;deletedID:id&nbsp;});<br />&nbsp;&nbsp;&nbsp;&nbsp;});<br />});<br /><br />app.listen(port,&nbsp;()&nbsp;=&gt;&nbsp;{<br />&nbsp;&nbsp;&nbsp;&nbsp;console.log('Server&nbsp;running&nbsp;at&nbsp;http://localhost:${port}');<br />});</span>
</code>  </code></pre><br>
...
<ul>
<li>Open Postman.</li>
<li>Set the request type to GET.</li>
<li>Enter the URL: <em>http://localhost:3000/students</em>.</li>
</ul>
<img src="nodejs/dbget.png" alt="image" class="img-rounded img-responsive"/><br/>

<h3>POST : Create a New Student</h3>
<ul>
<li>Open Postman.</li>
<li>Set the request type to POST.</li>
<li>Enter the URL: <em>http://localhost:3000/students.</em></li>
<li>Go to the <strong>"Body" </strong>tab.</li>
<li>Select raw and set the body to JSON format.</li>
</ul>
<img src="nodejs/dbpost.png" alt="image" class="img-rounded img-responsive"/><br/>
...
<li>You should receive a response with details of all students in your SQLite database.</li>
</ul>
<img src="nodejs/dbgetall.png" alt="image" class="img-rounded img-responsive"/><br/>

<h3>DELETE:</h3>
<ul>
<li>Set the request type to DELETE.</li>
<li>Enter the URL for the student you want to delete (replace: id with an actual student ID): <em>http://localhost:3000/students/:id</em></li>
<li>Place instead of ID which replace with number that is ID to be deleted.</li>
<li>Then click <strong>send</strong></li>
</ul>
<img src="nodejs/dbdel.png" alt="image" class="img-rounded img-responsive"/><br/>

<h3>PUT:</h3>
<ul>
<li>Set the request type to PUT.</li>
<li>Enter the URL for the student you want to delete (replace: id with an actual student ID): <em>http://localhost:3000/students/:id</em></li>
<li>Go to the <strong>"Body"</strong> tab.</li>
<li>Select raw and set the body to JSON format</li>
</ul>
<img src="nodejs/dbput.png" alt="image" class="img-rounded img-responsive"/><br/>

 </body>

<!-- Mirrored from studyglance.in/labprograms/nodejsdisplay.php?url1=nodejs/exp7.php&title=Develop%20an%20express%20web%20application%20that%20can%20interact%20with%20REST%20API%20to%20perform%20CRUD%20operations%20on%20student%20data.%20(Use%20Postman) by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 28 May 2025 03:48:52 GMT -->
</html>
