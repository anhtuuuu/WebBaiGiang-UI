...
<!-- Mirrored from studyglance.in/labprograms/nodejsdisplay.php?url1=nodejs/exp9.php&title=Create%20a%20TODO%20application%20in%20react%20with%20necessary%20components%20and%20deploy%20it%20into%20github. by HTTrack Website Copier/3.x [XR&CO'2014], Wed, 28 May 2025 03:48:54 GMT -->
...
<h2>Aim: </h2>
  <h1 style="font-size:18px;color:black;"><span style='font-size:25px;color:#ff3333;'>&#9755;</span> &nbsp;Create a TODO application in react with necessary components and deploy it into github. </h1>
  <h2>Solution : </h2>	
...
<p>Our first task is to set up the React project. This step involves creating the necessary project structure. Here's how you can do it:</p>
<h4>1. Create a React App:</h4>
<p>Open your terminal and navigate to your preferred directory. Run the following command to generate a new React app. Replace <strong>"todo-app"</strong> with your desired project name:</p>
<pre><code>
npx create-react-app todo-app

</code></pre>
<p>This command will create a directory named "todo-app" with all the initial code required for a React app.</p>
<h4>2. Navigate to the Project Directory:</h4>
<p>Change your working directory to the "todo-app" folder:</p>
<pre><code>
cd todo-app

</code></pre>
<h4>3. Start the Development Server:</h4>
<p>Launch the development server with the following command:</p>
<pre><code>
npm start

</code></pre>
<p>This will open your React app, and you’ll see the default React starter page in your web browser at <em>'http://localhost:3000'</em>.</p>
<h3>Step 2: Create the App Component</h3>
<p>In this step, we create the App component, which serves as the entry point to our Todo List application.</p>

<pre><code class="language-js"><code><span style="color: #000000">
import&nbsp;React&nbsp;from'react';<br />import&nbsp;TodoList&nbsp;from'./components/TodoList';<br />function&nbsp;App()&nbsp;{<br />return&nbsp;(<br />&lt;div&nbsp;className="App"&gt;<br />&lt;TodoList&nbsp;/&gt;<br />&lt;/div&gt;<br />&nbsp;);&nbsp;<br />}<br />exportdefault&nbsp;App;<br /></span>
</code>  </code></pre><br>
<h3>Step 3: Create the TodoList </h3>
<h4>src->Component</h4>

<p>Now, let's create the 'TodoList' component, which is responsible for managing the list of tasks and handling task-related functionality.</p>
<pre><code class="language-js"><code><span style="color: #000000">
import&nbsp;React,&nbsp;{&nbsp;useState&nbsp;}&nbsp;from&nbsp;'react';&nbsp;<br />import&nbsp;TodoItem&nbsp;from&nbsp;'./TodoItem';&nbsp;<br /><br />function&nbsp;TodoList()&nbsp;{<br />const&nbsp;[tasks,&nbsp;setTasks]&nbsp;=&nbsp;useState([<br />&nbsp;{<br />id:&nbsp;1,<br />text:&nbsp;'Doctor&nbsp;Appointment',<br />completed:&nbsp;true<br />&nbsp;},<br />&nbsp;{<br />id:&nbsp;2,<br />text:&nbsp;'Meeting&nbsp;at&nbsp;School',<br />completed:&nbsp;false<br />&nbsp;}<br />&nbsp;]);<br /><br />const&nbsp;[text,&nbsp;setText]&nbsp;=&nbsp;useState('');<br />function&nbsp;addTask(text)&nbsp;{<br />const&nbsp;newTask&nbsp;=&nbsp;{<br />id:&nbsp;Date.now(),<br />&nbsp;text,<br />completed:&nbsp;false<br />&nbsp;};<br />&nbsp;setTasks([tasks,&nbsp;newTask]);<br />&nbsp;setText('');<br />&nbsp;}<br />function&nbsp;deleteTask(id)&nbsp;{<br />&nbsp;setTasks(tasks.filter(task&nbsp;=&gt;&nbsp;task.id&nbsp;!==&nbsp;id));<br />&nbsp;}<br />function&nbsp;toggleCompleted(id)&nbsp;{<br />&nbsp;setTasks(tasks.map(task&nbsp;=&gt;&nbsp;{<br />if&nbsp;(task.id&nbsp;===&nbsp;id)&nbsp;{<br />return&nbsp;{task,&nbsp;completed:&nbsp;!task.completed};<br />&nbsp;}&nbsp;else&nbsp;{<br />return&nbsp;task;<br />&nbsp;}&nbsp;<br />&nbsp;}));<br />&nbsp;}<br />return&nbsp;(<br />&lt;div&nbsp;className="todo-list"&gt;<br />&nbsp;{tasks.map(task&nbsp;=&gt;&nbsp;(<br />&lt;TodoItem<br />&nbsp;key={task.id}&nbsp;<br />&nbsp;task={task}<br />&nbsp;deleteTask={deleteTask}<br />&nbsp;toggleCompleted={toggleCompleted}&nbsp;<br />&nbsp;/&gt;<br />&nbsp;))}<br />&lt;input<br />&nbsp;value={text}<br />&nbsp;onChange={e&nbsp;=&gt;&nbsp;setText(e.target.value)}&nbsp;<br />&nbsp;/&gt;<br />&lt;button&nbsp;onClick={()&nbsp;=&gt;&nbsp;addTask(text)}&gt;Add&lt;/button&gt;<br />&lt;/div&gt;<br />&nbsp;);<br />}<br />exportdefault&nbsp;TodoList;</span>
</code>  </code></pre><br>
<h3>Step 4: Create the place the TodoItem in </h3>
<h4>src->Component</h4>
<p>In this step, we create the 'TodoItem' component, which represents an individual task in our Todo List.</p>
<pre><code class="language-js"><code><span style="color: #000000">
import&nbsp;React&nbsp;from'react';<br />function&nbsp;TodoItem({&nbsp;task,&nbsp;deleteTask,&nbsp;toggleCompleted&nbsp;})&nbsp;{<br />function&nbsp;handleChange()&nbsp;{<br />&nbsp;toggleCompleted(task.id);<br />&nbsp;}<br /><br />return&nbsp;(<br />&lt;div&nbsp;className="todo-item"&gt;<br />&lt;input&nbsp;<br />&nbsp;type="checkbox"<br />&nbsp;checked={task.completed}<br />&nbsp;onChange={handleChange}<br />&nbsp;/&gt;<br />&lt;p&gt;{task.text}&lt;/p&gt;<br />&lt;button&nbsp;onClick={()&nbsp;=&gt;&nbsp;deleteTask(task.id)}&gt;<br />&nbsp;X<br />&lt;/button&gt;<br />&lt;/div&gt;<br />&nbsp;);<br />}<br />exportdefault&nbsp;TodoItem;</span>
</code>  </code></pre><br>
<p>These three components, 'App', 'TodoList', and 'TodoItem', work together to create a functional Todo List application in React. The 'TodoList' component manages the state of the tasks, and the 'TodoItem' component represents and handles individual tasks within the list.</p>
<h3>Step 5: Styling</h3>
<p>To enhance the visual appeal of your Todo List, you can apply some basic styling. Here’s an example of how you can style the todo items. In the `App.css` file, add the following styles:</p>
<pre><code class="language-css"><code><span style="color: #000000">
.todo-item&nbsp;{<br />display:&nbsp;flex;<br />justify-content:&nbsp;space-between;<br />margin-bottom:&nbsp;8px;<br />}<br />.todo-itemp&nbsp;{<br />color:&nbsp;#888;<br />text-decoration:&nbsp;line-through;<br />}</span>
</code>  </code></pre><br>

<p>Your output should look like this:</p>
<h2> Output : </h2>
<h4> Initially it looks like: </h4>
<img src="nodejs/todo1.png" alt="image" class="img-rounded img-responsive"/><br/>
<h4> Next, </h4>
<img src="nodejs/todo2.png" alt="image" class="img-rounded img-responsive"/><br/>
 </body>
